...
class Multisig
{
    ...
    public static function fromDecodedScript(array $decoded, PublicKeySerializerInterface $pubKeySerializer = null, $allowVerify = false)
    {
        ...
        $requiredSigs = \Barechain\Bitcoin\Script\decodeOpN($mCode);
        $publicKeyBuffers = [];
        foreach (array_slice($decoded, 1, -2) as $key) {
            /** @var \Barechain\Bitcoin\Script\Parser\Operation $key */
            if (!$key->isPush()) {
        ...
        $keyCount = \Barechain\Bitcoin\Script\decodeOpN($nCode);
